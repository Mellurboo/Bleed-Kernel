...
static uint64_t alloc_empty_frame(void **vaddr) {
    uint64_t paddr = (uint64_t)alloc_pages(1);
    if(!paddr)
        kprintf(LOG_ERROR "Page Allocation Failed\n");
    uint64_t phys = paddr & PADDR_ENTRY_MASK;

    void* v = paddr_to_vaddr((void *)(uintptr_t)phys);
...
void walk_page_tables(uint64_t vaddr, uint64_t **out_pd, size_t *out_pd_index) {
    ...
    uint64_t* pml4_vaddr = (uint64_t*)paddr_to_vaddr((void *)(uintptr_t)cr3);
    ...
    uint64_t* pdpt = (uint64_t*)paddr_to_vaddr((void *)(uintptr_t)p_pdpt);

    uint64_t p_pd = write_table_entry(pdpt, pdpt_index, PTE_PRESENT | PTE_WRITABLE);
    uint64_t* pd = (uint64_t*)paddr_to_vaddr((void *)(uintptr_t)p_pd);
...
void extend_paging() {
    ...
    for (size_t i = 0; i < mmap->entry_count; i++) {
        ...
        for (uint64_t paddr = base; paddr + PAGE_SIZE_2M <= end; paddr += PAGE_SIZE_2M) {
            void* hv = paddr_to_vaddr((void *)(uintptr_t)paddr);
            if (!hv) continue;

            map_page(paddr, (uint64_t)(uintptr_t)hv, 0);
            mapped++;
        }
    }
